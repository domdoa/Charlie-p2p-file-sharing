...
public class Node {
    ...
    public void downloadFromMultiplePeers(List<InetSocketAddress> peersAddresses, File fileDataToRead, String path) throws IOException {
        List<Socket> sockets = new ArrayList<>();
        for (InetSocketAddress inetSocketAddress : peersAddresses) {
            ServerSocket serverSocket = listener.getServerSocket();
            Socket socket = serverSocket.accept();
            sockets.add(socket);
        }
        int numberOfPeers = peersAddresses.size();
        int bytesPerPeer = fileDataToRead.getSize() / numberOfPeers;

        int i = 0;
        ThreadPoolExecutor threadPoolExecutor = (ThreadPoolExecutor) Executors.newFixedThreadPool(numberOfPeers);
        for (Socket socket : sockets) {

            int finalI = i;
            threadPoolExecutor.submit(()->{
                java.io.File localFile = new java.io.File(path + finalI + "." + fileDataToRead.getExtension());
                Path pathToFile = Paths.get(localFile.toURI());
                Charset charset = Charset.forName("US-ASCII");
                ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
                try {

                    InputStream in = socket.getInputStream();
                    byte[] bytes = new byte[bytesPerPeer];
                    int nRead = 0;
                    while((nRead = in.read(bytes, finalI * bytesPerPeer, (finalI+1)*bytesPerPeer))!=-1){
                        byteArrayOutputStream.write(bytes,finalI*bytesPerPeer,nRead);
                    }
                ...
                try (BufferedWriter writer = Files.newBufferedWriter(pathToFile, charset)) {
                    writer.write(byteArrayOutputStream.toString(), 0, bytesPerPeer);
                } catch (IOException x) {
    ...
    public String notify(InetSocketAddress successor) {
        if (successor != null && !successor.equals(localAddress))
            return Helper.sendRequestAndReceiveResponse(successor, "IAMPRE_" + localAddress.getAddress().toString() + ":" + localAddress.getPort());
        else
            return null;
    }
    ...
    public InetSocketAddress closest_preceding_finger(long findid) {
        ...
        for (int i = 32; i > 0; i--) {
            ...
            if (ith_finger_relative_id > 0 && ith_finger_relative_id < findid_relative) {
                String response = Helper.sendRequestAndReceiveResponse(ith_finger, "KEEP");
