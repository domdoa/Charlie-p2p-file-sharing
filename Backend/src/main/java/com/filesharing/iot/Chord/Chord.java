...
public class Chord {
    ...
    private static ForeignPC m_contact = new ForeignPC(null, "");
    ...
    public static void main(String[] args) {
        ...
        // create node
        String springPort = Constants.currentSpringPort;
        m_node = new Node(new ForeignPC(Helper.createSocketAddress(local_ip + ":" + args[0] + ":" + springPort), springPort));

        // determine if it's creating or joining a existing ring
        // create, contact is this node itself
        if (args.length == 1) {
            m_contact.setInetSocketAddress(m_node.getLocalAddress().getInetSocketAddress());
        }

        // join, contact is another node
        else if (args.length == 3) {
            m_contact.setInetSocketAddress(Helper.createSocketAddress(args[1] + ":" + args[2]));
           // m_contact.setSpringPort();
            if (m_contact == null) {
